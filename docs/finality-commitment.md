...
| Agreement discriminator, Listing pin, fixed/negotiable-centre or exact metered total, rail, full-JCS `DeliverableSpec` hash, positional buyer/seller parties, pattern, and payout coverage gate before submission; unknown pricing records `unrecognized-pricing-kind` | DACS-3 §8.5.2 checks 1–4, 7, and 9; MTR-1–MTR-5; §8.6 step 3; DACS-4 §9.3; CA-5 |
| Auto-accept fails closed until the SDK has typed verification of the anchored template commitment and live per-instance seller signature | DACS-3 §8.4.1 seller-side auto-accept; §8.5.2 ordering note |
| Logical address is exactly `dacs3:commit:{jobId}`, with `jobId` accepted only in canonical uppercase ULID form | DACS-3 §8.6 step 5; CORE §B.1 CF-4 |
| An existing record must bind the same immutable session content and is never replaced by another commitment type/content; retry-time expiry does not invalidate a record whose authenticated finality timestamp was in time | DACS-3 §8.5.2 checks 5–6 and ordering note; §8.6 CA-3 |
