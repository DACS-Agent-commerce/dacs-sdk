...
This audit uses the adopted DACS Standard `next` at [commit
`741057bf26509ca2701ee78171e6049b1bc665b4`](https://github.com/DACS-Agent-commerce/DACS-Standard/tree/741057bf26509ca2701ee78171e6049b1bc665b4)
as the exact reviewed normative source. The SDK conformance runner is
reproducibly pinned to that revision by `scripts/sync-vectors.mjs`.
...
| Replay, retry, and recovery | DACS-4 §9.5.1 PC-7, §9.5.6 AP2-5/AP2-6, §9.5.7 X402 retry rules, §9.5.8 SB-1..SB-3; DACS-5 §10.3.1 ST-1, ST-7..ST-11; DACS-5 §10.11 | Persist exact session/listing/agreement/payment/delivery identities; enforce single-use settlement identifiers; reconcile ambiguous external effects before retry; never repeat payment or fulfilment after restart; resume only the recorded forward state; keep finalization pending until all required evidence is finalized and resolvable. CORE §B.8 instead governs verifier-issued identity-presentation nonces and is not the authority for this seller lifecycle. | `SessionStore` primitives and buyer-side integration exist, but `createAgent().runSession()` does not expose durable-store wiring and there is no seller lifecycle. | #55 (seller recovery), #33 (settlement uniqueness), #81 (bundle finalization), #92 (atomic rollback/idempotency). |
...
| `resolveListingRails` | DACS-1 §6.3.4 LRR-1..LRR-6; DACS-4 §9.4.3 RD-1..RD-6 | All 29 `listing-rail-registry-resolution-v0.4` vectors from adopted Standard `next` commit `741057bf26509ca2701ee78171e6049b1bc665b4` |
...
| `createPayDemSellerObserver` | DACS-4 §9.5.1 PC-1..PC-7; §9.5.9; §9.7 (`observedAt`, `finalityObservedAt`) | Reads status, transaction, and confirmed block through the public Demos RPC; requires one matching hash/block/native-send body, with authoritative status `included` and a body label that represents the same included finality (`included`, `confirmed`, or `finalized`). The native-send payload disambiguates legacy numeric DEM from post-fork string OS; a current-node numeric `content.amount` projection is treated as OS only when it exactly matches the string payload. The payer is the ed25519 owner account whose nonce/balance edits execute, rather than an alternate active signing key. Integer-only comparison and the consensus block timestamp gate amount and finality. Contradictory facts reject, while unavailable reads remain retryable. RPC duration and decoded body bytes are bounded as an operational resource guard. This is a configured-node trust boundary, not an independent validator-quorum proof. |
...
| SDK surface | Normative source | Enforced behavior / evidence |
| --- | --- | --- |
| Payment-to-delivery gate | DACS-4 §9.5.1 PC-2/PC-7; §9.5.8 SB-1/SB-2; §9.9 PIPE-1/PIPE-3/PIPE-4/PIPE-6; DACS-5 §10.3 | This focused release implements only a pay-then-deliver subset: one successful payment phase after one payee-bound commitment must precede exactly one bound delivery phase. Zero-pay and deliver-then-pay pipelines remain normative but unsupported here; repeated payment/delivery phases also fail closed pending #17/DACS-Standard #329. The core validates the exact finalized commitment, its authenticated signer against the SessionRecord orchestrator, Listing, rail snapshot, payment identity/evidence, and contiguous prior `SessionRecord.phaseResults`. PC-7 pointerless payment catch-up remains legal: the independently authenticated authorization assigns its one exact canonical evidence ref without rewriting the SessionRecord. The core derives a phase-indexed fulfilment id, then atomically commits permit consumption with the complete validated candidate. A consumed replay reconciles first and may resume only those exact retained bytes/hash under the same idempotency key. |
...
| Recovery and audit-source seam | DACS-5 §10.3.1 ST-1/ST-7/ST-11 | Every application call receives the canonical-object v2 fulfilment id, binding job, payment/delivery indices, settlement, agreement, and payment evidence without delimiter ambiguity. All callback and receipt-store methods are captured and bound synchronously before the first await, so later getter/proxy/method swaps cannot change authority. The V2-only profile resolves one atomic, exact-JCS pre-delivery source containing the authenticated SessionRecord, finalized commitment ref, party Vet refs plus operational requirement invocations, and exact settlement inventory. Before permit consumption, the authenticated orchestrator signs a domain-separated commitment over the fulfilment id, job, authorization hash, exact source hash, and retained candidate hash; replay verifies that commitment before any effect. Consumption atomically retains the commitment, source, authorization, and complete candidate/failure handoff. Delivery SettlementEvidence then independently signs the same source hash, protecting both the pre-evidence recovery window and the terminal record. The pre-repair V1 draft handoff was never released and is deliberately rejected because it cannot support a WAL-derived authenticated audit projection; callers holding draft data must restart before consumption. Complementary Vet requirement bodies/verifier identities are explicitly operational provenance pending DACS-Standard #331, not claimed as normative VPC-2 authority. #121 must inherit and generation-fence this authenticated handoff and expose the resulting WAL-only projection. |
