...
| Recovery seam | DACS-5 §10.3.1 ST-1/ST-7/ST-11 | Every application call receives the canonical-object v2 fulfilment id, binding job, payment/delivery indices, settlement, agreement, and payment evidence without delimiter ambiguity. All callback and receipt-store methods are captured and bound synchronously before the first await, so later getter/proxy/method swaps cannot change authority. Consumption atomically retains the exact authorization plus complete candidate/failure handoff; response loss therefore cannot strand a paid job between permit use and delivery. Recovery never calls the preparer again, validates every retained binding/byte, reconciles first, and permits only an exact idempotent submit. #121 must add durable storage and generation-fenced ownership around this retained handoff, not derive authority from caller input. |
