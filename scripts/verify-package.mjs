...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { canonicalDemosAgentPublicKey, demosAgentClaimRef, demosAgentPublicKey, parseDemosAgentClaimReference } from "@kynesyslabs/dacs/identity";',
      ...
      'if (parseDemosAgentClaimReference(`DID:${demosClaim.slice(4)}?a=x%3Ay`)?.canonicalIdentity !== demosClaim) throw new Error("identity parse failed");',
      'if (canonicalDemosAgentPublicKey(`DID:${demosClaim.slice(4)}`) !== null) throw new Error("signed identity repair leaked");',
