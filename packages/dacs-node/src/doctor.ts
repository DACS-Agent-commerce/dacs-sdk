...
export interface DacsLiveDoctorCheckV1 {
  ...
  category: "local" | "demos" | "x402" | "service";
...
const DEFINITIONS: readonly Readonly<CheckDefinition>[] = Object.freeze([
  ...
  { id: "x402.rail-authority", label: "authenticated x402 rail authority", availableFrom: "pre-start", scopes: ALL_SCOPES },
  { id: "x402.testnet-policy", label: "non-mainnet release policy", availableFrom: "pre-start", scopes: ALL_SCOPES },
  { id: "x402.endpoints", label: "x402 facilitator and resource endpoints", availableFrom: "pre-start", scopes: ALL_SCOPES },
  { id: "x402.token-domain", label: "payment token EIP-712 domain", availableFrom: "pre-start", scopes: ALL_SCOPES },
  { id: "x402.payer-binding", label: "buyer EVM payer binding", availableFrom: "pre-start", scopes: BUY },
  { id: "x402.payee-binding", label: "seller EVM payee binding", availableFrom: "pre-start", scopes: SETUP_BUY },
  { id: "x402.asset-balance", label: "payment-asset balance", availableFrom: "pre-start", scopes: BUY },
  { id: "x402.gas-balance", label: "native gas balance", availableFrom: "pre-start", scopes: BUY },
  { id: "x402.service-limit", label: "configured maximum service amount", availableFrom: "pre-start", scopes: BUY },
  { id: "x402.cost-estimate", label: "service and network cost estimate", availableFrom: "pre-start", scopes: BUY },
...
function requiredFor(
  ...
  return definition.scopes.has(scope) &&
...
async function runProbe(
  ...
  const startedAt = now();
  const required = requiredFor(definition, options.phase, options.scope);
