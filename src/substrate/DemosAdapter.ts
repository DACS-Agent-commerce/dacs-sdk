...
import { canonicalize, logicalToStorageProgramName } from "../canonical/index.js";
...
export class DemosAdapter implements SubstrateAdapter {
  ...
  async anchor(name: string, value: object, options: AnchorOptions = {}): Promise<AnchorRef> {
    ...
    // own program by name first; update it in place if present. Per-job
    // deterministic slots can explicitly assert that they are known-new and
    // avoid waiting for the eventually-consistent name index.
    const self = this.demos.getAddress();
    const resolution: AnchorResolution = options.writeMode === "known-new"
      ? { status: "absent" }
      : await this.resolveAnchorByName(name, self);
    ...
    let creating = false;
    let reservedNonce = options.nonce;
    if (resolution.status === "present") {
      ...
      // program lands at is derived from that same nonce. A caller-supplied
      // reservation and the transaction signer MUST use this exact value.
      reservedNonce ??= await this.nextAnchorNonce();
      address = StorageProgram.deriveStorageAddress(self, programName, reservedNonce, ANCHOR_SALT);
      payload = StorageProgram.createStorageProgram(
        ...
        { nonce: reservedNonce, salt: ANCHOR_SALT },
      );
      creating = true;
    }
    ...
    if (broadcast?.result !== 200 && options.writeMode === "known-new" && creating) {
