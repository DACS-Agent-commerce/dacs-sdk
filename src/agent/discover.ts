  ...
  const verdict = await verifyComponentSignature(
    ...
    ARTIFACT_SEPARATORS.Listing,
    {
      ...
      resolvePublicKey: async (signature) => {
        const parsedSigner = parseCanonicalClaimReference(signature.signer);
        if (parsedSigner === null) return null;
        const resolved = await resolveKey(
          `${parsedSigner.identity.scheme}:${parsedSigner.identity.identifier}`,
        );
